...
> **Command:** `.claude/commands/run-spec.md`
...
> **Stack:** resolve every runtime command from the target project — `.claude/skills/00-openspec-stack-agnostic/SKILL.md`
...
Each phase runs in a **fresh-context subagent** (`Agent` tool, `subagent_type: general-purpose`) so the orchestrator's context stays small and each phase re-derives its own facts.
...
- Otherwise delegate to `.claude/skills/prepara-tarea/SKILL.md` with the verbatim description.
...
- Delegate to `.claude/skills/repasa-spec/SKILL.md` (gaps → mejora → aplica → gaps, sequential).
...
- Delegate to `.claude/commands/ejecuta-tests-reporte.md` — the **narrowest** project test command that covers the change (derived from `tasks.md` + `specs/**`).
- Exit 0 → `done` (record the exact command and exit code).
- Non-zero → classify per `.claude/skills/openspec-e2e-regression-guard/SKILL.md`:
...
3. Persist state.
4. Yield with `ScheduleWakeup`:

| Situation | `noop` | `delaySeconds` | `stop` |
|-----------|-------:|---------------:|-------:|
| A phase advanced the state | `false` | 60 (next phase can start now) | — |
| Waiting on an external signal (deploy CI, remote queue) | `true` | matched to that wait (e.g. ~480 for an ~8 min CI) | — |
| Terminal verdict reached, **including `AWAITING DEPLOY APPROVAL`** (a tick cannot ask the developer) | — | — | `true` |

- `prompt` = the same `/run-spec <slug>` text, verbatim, every tick.
- `reason` names what is actually being awaited ("waiting on deploy CI", not "waiting").
- **Never** schedule short polls for harness-tracked subagents — completed subagents notify on their own; use a long fallback instead.
- In **Cursor**, use the `/loop` monitored-shell mechanism (unique `AGENT_LOOP_WAKE_<slug>` sentinel + JSON prompt payload) with the same one-phase-per-tick contract.
...
| Path | Role |
|------|------|
| `.claude/commands/run-spec.md` | Slash command entry point |
| `.claude/skills/prepara-tarea/SKILL.md` | P1 |
| `.claude/skills/repasa-spec/SKILL.md` | P2, P3, P4 |
| `.claude/commands/ejecuta-tests-reporte.md` | P5 |
| `.claude/skills/openspec-e2e-regression-guard/SKILL.md` | P5/P6 failure classification |
| `.claude/skills/multiagente/SKILL.md` | Unattended orchestration contract |
| `.claude/skills/00-openspec-stack-agnostic/SKILL.md` | Resolve project test/E2E/deploy commands |
