...
> **Command:** `.cursor/commands/comando-verificar.md`  
> **Rule (L2):** `.cursor/rules/comando-verificar-a-fondo.mdc`  
...
- Local stack running for tests/CLI when rows need runtime — resolve from target project stack rule (`00-openspec-stack-agnostic.mdc`); do not assume DDEV.
...
**Form event map:** update `.cursor/context/testing/form-event-map.md` when form scope is non-trivial (REQ-CV-MATRIX-04).
...
Subfolder **`verification/`** lives under `.cursor/context/` per `ai-helper-files.mdc`.
...
Matriz per-task: `.cursor/context/verification/` — **not** the same as multi-role HTML QA reports.
