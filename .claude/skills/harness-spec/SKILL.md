...
Comprobar con comandos reales antes de decidir, no asumir: `openspec status --change "<slug>" --json`, `openspec validate "<slug>"`, `git log --oneline -- openspec/changes/<slug>` o `openspec/specs/<capability>`, y si existe, la última matriz en `.claude/context/verification/openspec-<slug>/matrix.md` o `.claude/context/verification/openspec-<slug>/matrix.md`.
