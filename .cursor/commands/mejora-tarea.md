...
If **`GAPS.md`** exists with **FACT** gaps pending hydration, incorporate them into `specs/**/spec.md` as formal requirements before marking READY. Re-read matrix rows marked pending or `(proposed REQ-…)` — do not trust prior HYDRATED claims without `grep` on the spec.
