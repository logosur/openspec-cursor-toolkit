...
> **Rule (L2):** `.claude/skills/qa/SKILL.md`  
...
- Local stack running — resolve from target project stack rule (`.claude/skills/00-openspec-stack-agnostic/SKILL.md`).
...
**Form event map:** update `.claude/context/testing/form-event-map.md` when form scope is non-trivial.

---

## Phase 2 — Combinatorial matrix

Path: **`.claude/QAtest/<run-slug>/matrix.md`**
...
```bash
bash .claude/scripts/open-html-in-chrome.sh \
  "$(git rev-parse --show-toplevel)/.claude/QAtest/<run-slug>/qa-report-<run-slug>.html"
```
...
| Open report | `bash .claude/scripts/open-html-in-chrome.sh <path>` |
...
| `/verifica-tarea` | OpenSpec compliance + matrix under `.claude/context/verification/` |
