...
1. Leer y seguir **`.claude/skills/cierra-sesion/SKILL.md`**.
...
3. Si queda una **duda que solo el desarrollador puede resolver**, abrir un diálogo de preguntas (`AskUserQuestion`) **antes** de dar el veredicto — nunca adivinar ni dar por cerrado lo dudoso.
...
Si el estado de algo **no se puede determinar leyendo el repo ni el hilo**, no se asume: se abre `AskUserQuestion` con la pregunta en lenguaje natural y opciones bien explicadas (qué significa elegir cada una y qué pasa después). Casos típicos:
...
| Item | Ruta |
|------|------|
| Skill | `.claude/skills/cierra-sesion/SKILL.md` |
| Sumario de spec | `.claude/commands/sumario-spec.md` |
| Cierre de spec | `.claude/commands/finaliza-spec.md` |
