...
**Regla:** si el mensaje encaja con **cualquier** frase de la tabla (intención de gap analysis / GAPS.md / casuísticas faltantes), el agente **debe** cargar y seguir **`.cursor/skills/openspec-gap-analysis/SKILL.md`** aunque no aparezca `/gaps-spec`.
...
2. Leer y seguir **`.cursor/skills/openspec-gap-analysis/SKILL.md`** (paso a paso).
3. Aplicar **`.cursor/rules/openspec-gap-analysis.mdc`** (taxonomía FACT/INFERENCE/REJECTED, bucle ≤5 iter).
...
| Artefacto | Rol |
|-----------|-----|
| `.cursor/skills/openspec-gap-analysis/SKILL.md` | Workflow ejecutable |
| `.cursor/rules/openspec-gap-analysis.mdc` | Reglas L2 (globs OpenSpec) |
| `.cursor/rules/hazlo-en-modo-multiagente.mdc` | Orquestación multiagente |
