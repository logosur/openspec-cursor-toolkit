...
> **Command:** `.cursor/commands/verifica-tarea.md`  
> **Rule (L2):** `.cursor/rules/verifica-tarea-openspec-a-fondo.mdc`  
...
- **Local stack** running for tests/CLI when rows need runtime — resolve commands from the target project's stack rule (`00-openspec-stack-agnostic.mdc`); do not assume DDEV/Drupal.
...
5. **Form event map:** for non-trivial **form** scenarios on product changes, update `.cursor/context/testing/form-event-map.md` (REQ-VT-MATRIX-08).
...
Path: **`.cursor/context/verification/openspec-<slug>/matrix.md`**
...
| L1 budget | `bash .cursor/scripts/count-always-on-rules.sh` when present |
