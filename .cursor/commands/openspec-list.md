...
```bash
bash .cursor/scripts/openspec-list.sh
```

Con conteo explícito:

```bash
bash .cursor/scripts/openspec-list.sh 20
```

El script **siempre**:

1. Imprime la tabla Markdown en stdout (para pegar en el chat).
2. Escribe `.cursor/context/openspec/openspec-list.html` (o `--html [path]`).
...
| Flag | Efecto |
|------|--------|
| `--html [path]` | Ruta del HTML (default: `.cursor/context/openspec/openspec-list.html`) |
...
- `.cursor/context/openspec-docs/README.md` — índice OpenSpec del proyecto.
