...
> **Command:** `.cursor/commands/supervisa-sesiones.md`  
> **Rule:** `.cursor/rules/supervisa-sesiones-multisesion.mdc`
> **Registro de estado:** `.cursor/context/supervisor/SUPERVISOR-<YYYYMMDD-HHMM>.md` (en el repo desde el que se invoca)
...
> **Stack:** cada comando de runtime se resuelve en **el proyecto destino** — `.cursor/skills/00-openspec-stack-agnostic/SKILL.md`

> **Portabilidad Cursor.** Los nombres de herramientas citados abajo (`mcp__ccd_session_mgmt__*`,
> `ListAgents`, `SendMessage`, `Agent`, `Monitor`, `TaskStop`, `ScheduleWakeup`)
> son los del harness de Claude Code. En Cursor normalmente **no hay canal de sesiones**: se
> arranca en el **modo manual de §9** —se dice en la primera línea— y el informe de liberación de
> F4b se mantiene igual: el supervisor **te dice qué sesión está libre y qué entregó**, y nunca la
> cierra ni la archiva por ti — eso lo decides tú después. El cerrojo de §0 y el watchdog de §W
> funcionan igual: son ficheros y plazos, no herramientas — sin API de sesiones, la comprobación de
> «¿el dueño sigue vivo?» se hace por latido y preguntándote, nunca reclamando a la brava.
...
| Canal de diálogo con el desarrollador (AskQuestion en Cursor, o pregunta explícita en el chat) | La skill **no puede** cumplir su contrato sin poder preguntar; sin él acaba en `AWAITING DEVELOPER DECISION` con la lista de preguntas escrita |
...
Crea `.cursor/context/supervisor/SUPERVISOR-<YYYYMMDD-HHMM>.md` con el esquema de §2. **Se escribe después de cada transición**: es la única memoria del supervisor y debe sobrevivir a compactación, a reinicio y a ticks de `/loop`.
...
- Diálogo (AskQuestion en Cursor, o pregunta explícita en el chat), **agrupando hasta 4 preguntas por diálogo**, en tandas, no de una en una.
...
| Documentos | solo el registro en `.cursor/context/supervisor/`; ningún `.md` de análisis suelto ni en la raíz |
