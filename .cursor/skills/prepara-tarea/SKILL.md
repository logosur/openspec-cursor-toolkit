...
- **HYDRATE** here means **OpenSpec markdown artefacts** under `openspec/changes/<slug>/` only (proposal, design, specs, tasks, VERIFY, READY-TO-APPLY, etc.) — **not** executing product work from `tasks.md` (no edits to application source outside `openspec/changes/<slug>/`; see `00-openspec-stack-agnostic.mdc`) in this turn.
